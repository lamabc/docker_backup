...
NOT_LESS_THEN_SIZE_TO_PERFORM_DELETE=410163640
...
if [ -d "$DIR_EX_DRIVE" ]; then
  ...
  echo "sudo cp -r  ${CONT_FOLDER}* ${DIR_EX_DRIVE} " >> execute_backup.sh
#  echo "sudo cp -r  ${CONT_FOLDER}IOTstack_${DATE}.tar.gz ${DIR_EX_DRIVE} " >> execute_backup.sh
  echo "cp ${CONT_FOLDER}haproxy_${DATE}.cfg ${DIR_EX_DRIVE} " >> execute_backup.sh 
else
